  ...
  if (!data || data.nodes.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-text-placeholder">No graph data yet. Process signals to build the knowledge graph.</p>
      </div>
    )
  }
  ...
  return (
    <div className="relative" style={{ height: '500px' }}>
      ...
      <div className="absolute top-2 left-2 z-10 flex items-center gap-2 bg-white/90 backdrop-blur rounded-lg px-3 py-1.5 shadow-sm border border-gray-200">
        ...
        <label className="flex items-center gap-1.5 text-[11px] text-text-placeholder cursor-pointer">
          <input
            type="checkbox"
            checked={showLabels}
      ...
      {showTip && (
        <div className="absolute top-14 left-2 z-10 bg-white rounded-lg shadow-lg border border-gray-200 px-3 py-2 max-w-xs">
          <p className="text-[11px] text-text-body mb-1.5">
      ...
      {selectedNode && (
        <div className="absolute top-0 right-0 w-72 h-full bg-white border-l border-gray-200 shadow-lg z-20 overflow-y-auto">
          <div className="p-4">
            ...
            <p className="text-[12px] text-text-body mb-3 whitespace-pre-wrap">
              {selectedNode.content}
            </p>

            <div className="space-y-1.5 text-[11px] text-text-placeholder mb-4">
              {selectedNode.node_type !== 'fact' && (
                <p>Confidence: <span className="font-medium text-text-body">{Math.round(selectedNode.confidence * 100)}%</span></p>
              )}
              {selectedNode.belief_domain && (
                <p>Domain: <span className="font-medium text-text-body">{selectedNode.belief_domain.replace('_', ' ')}</span></p>
              )}
              {selectedNode.insight_type && (
                <p>Type: <span className="font-medium text-text-body">{selectedNode.insight_type}</span></p>
              )}
              {selectedNode.source_type && (
                <p>Source: <span className="font-medium text-text-body">{selectedNode.source_type}</span></p>
              )}
              <p>Created: <span className="font-medium text-text-body">{new Date(selectedNode.created_at).toLocaleDateString()}</span></p>
            </div>
                ...
                <div className="space-y-1.5">
                  {connectedNodes.map(({ node, edgeType }) => (
                    <button
                      ...
                      className="w-full text-left bg-surface-muted rounded px-2.5 py-2 hover:bg-gray-100 transition-colors"
                    >
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        <span className="text-[9px] text-text-placeholder">{edgeType}</span>
                      </div>
                      <p className="text-[11px] text-text-body truncate">{node.summary}</p>
                    </button>
                  ))}
                </div>
              </div>
