...
export function ConnectionsTab({ data }: ConnectionsTabProps) {
  if (!data || data.nodes.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-ui-supportText">No connections to display yet.</p>
      </div>
    )
  }
  ...
  if (beliefs.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-sm text-ui-supportText">No beliefs formed yet. Process more signals to build connections.</p>
      </div>
    )
  }
    ...
    <div className="bg-ui-background rounded-lg overflow-hidden">
      ...
      >
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm font-medium text-ui-headingDark">{belief.summary}</p>
        </div>
        {expanded ? (
...
function ChainSection({ title, titleColor, borderColor, icon, edges, nodeMap, directionKey }: {
  ...
  return (
    <div className="mt-3">
      ...
      {edges.length === 0 ? (
        <p className="text-[11px] text-ui-supportText pl-5">None</p>
      ) : (
          ...
          {edges.map((edge) => {
            ...
            return (
              <div key={edge.id} className="bg-white rounded-lg px-3 py-2">
                ...
                <p className="text-sm text-ui-bodyText">{linkedNode.summary}</p>
