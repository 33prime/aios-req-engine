  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-3">
        ...
        <p className="text-xs text-ui-supportText">Analyzing project intelligence...</p>
      </div>
    )
  }
  ...
  if (!hasGaps && data.suggested_sources.length === 0 && data.tribal_knowledge.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-2">
        ...
        <p className="text-sm font-medium text-ui-headingDark">No significant gaps detected</p>
        <p className="text-xs text-ui-supportText text-center max-w-[240px]">
...
function SummaryBanner({ data }: { data: RequirementsIntelligenceResponse }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-ui-background rounded-lg px-3 py-2.5">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[11px] font-medium text-ui-bodyText">{readinessPercent}%</span>
      </div>
      <span className="text-[11px] text-ui-supportText flex-1 text-right truncate">
        {data.summary}
      </span>
    </div>
  )
  ...
  return (
    <div>
      <SectionHeader
        ...
        right={<span className="text-[11px] text-ui-supportText">{severitySummary}</span>}
      />
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h5 className="text-sm font-medium text-ui-headingDark">{gap.title}</h5>
                    {isFoundation && (
                  ...
                  <p className="text-xs text-ui-bodyText mt-0.5">{gap.description}</p>
                  {gap.how_to_fix && (
                    <p className="text-[11px] text-ui-supportText italic mt-1">
  ...
  return (
    <div>
      <SectionHeader
        ...
        right={<span className="text-[11px] text-ui-supportText">Key documents and artifacts to pursue</span>}
      />
          ...
          return (
            <div key={i} className="bg-gray-50 border border-gray-100 rounded-lg px-3 py-2.5">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <h5 className="text-xs font-medium text-ui-headingDark leading-tight">{src.title}</h5>
                  <p className="text-[11px] text-ui-bodyText mt-0.5">{src.description}</p>
          ...
          <div className="space-y-2">
            {known.map((s, i) => {
              ...
              const typeBadge = s.stakeholder_type
                ...
                : null
              return (
                <div key={s.stakeholder_id || i} className="bg-ui-background rounded-lg px-3 py-2.5">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-medium text-ui-headingDark">{s.name || 'Unknown'}</span>
                        ...
                        {s.influence_level && (
                          <span className="text-[9px] text-ui-supportText">
                      ...
                      {s.likely_knowledge.length > 0 && (
                        <p className="text-[11px] text-ui-bodyText mt-1">
                          <span className="text-ui-supportText">Likely knows about: </span>
                          {s.likely_knowledge.join(', ')}
                        </p>
                      )}
                      {s.engagement_tip && (
                        <p className="text-[10px] text-ui-supportText italic mt-1">{s.engagement_tip}</p>
                      )}
                    </div>
                  </div>
                </div>
            ...
            {suggested.map((s, i) => (
              <div key={i} className="border border-dashed border-gray-300 rounded-lg px-3 py-2.5">
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium text-ui-headingDark">{s.role}</span>
                      <span className="text-[9px] text-ui-supportText italic">Not yet identified</span>
                    </div>
                    {s.likely_knowledge.length > 0 && (
                    ...
                    {s.engagement_tip && (
                      <p className="text-[11px] text-ui-bodyText mt-1">{s.engagement_tip}</p>
                    )}
                  </div>
                </div>
              </div>
  ...
  return (
    <div>
      <SectionHeader
        ...
        right={<span className="text-[11px] text-ui-supportText">Knowledge that only comes from people</span>}
      />
      <div className="space-y-2">
        {items.map((item, i) => (
          <div key={i} className="bg-gray-50 border border-gray-100 rounded-lg px-3 py-2.5">
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                <h5 className="text-xs font-medium text-ui-headingDark">{item.title}</h5>
                <p className="text-[11px] text-ui-bodyText mt-0.5">{item.description}</p>
                <p className="text-[10px] text-ui-supportText italic mt-1">{item.why_undocumented}</p>
                ...
                {item.conversation_starters.length > 0 && (
                  <ul className="mt-2 space-y-1">
                    {item.conversation_starters.map((starter, j) => (
                      <li key={j} className="text-[11px] text-ui-bodyText pl-2 border-l-2 border-teal-200">
                        &ldquo;{starter}&rdquo;
                      </li>
                    ))}
                  </ul>
