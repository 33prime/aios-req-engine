    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          <Loader2 className="h-6 w-6 text-brand-primary mx-auto mb-2 animate-spin" />
          <p className="text-xs text-text-placeholder">Loading collaboration...</p>
        </div>
      </div>
    )
  }
  ...
  if (!progress) {
    return (
      <div className="p-4 text-center">
        <p className="text-xs text-text-placeholder mb-2">Could not load collaboration data</p>
        <button onClick={loadData} className="text-xs text-brand-primary hover:text-[#25785A] font-medium">
          Retry
        </button>
      </div>
    )
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="p-3 space-y-3">
          ...
          {gatesTotal > 0 && (
            <div className="flex items-center gap-1 mt-2">
              {gates.map((gate: PhaseGate, i: number) => (
                <div
                  key={i}
                  className={`w-2 h-2 rounded-full ${gate.met ? 'bg-brand-primary' : 'bg-border'}`}
        ...
        {/* Next Action Card */}
        <div className="border border-brand-primary/30 bg-brand-primary-light rounded-lg p-3">
          <div className="flex items-center gap-1.5 mb-1.5">
            <Target className="h-3.5 w-3.5 text-brand-primary" />
            <span className="text-[11px] font-semibold text-brand-primary uppercase tracking-wide">Next Step</span>
          </div>
          <p className="text-sm font-medium text-text-body mb-1">
            {actionWithCta.label}
          </p>
          <p className="text-xs text-text-placeholder mb-2">
          ...
          {showInviteInput && actionWithCta.type === 'invite_client' && (
            <div className="flex items-center gap-1.5 mb-2">
              <input
                ...
                className="flex-1 px-2.5 py-1.5 border border-border rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-brand-primary"
                disabled={inviting}
              />
              <button
                onClick={handleInviteClient}
                disabled={!inviteEmail.trim() || inviting}
                className="px-2.5 py-1.5 bg-brand-primary text-white text-xs font-medium rounded-lg disabled:opacity-50 hover:bg-[#25785A] transition-colors"
              >
          ...
          {actionWithCta.cta && actionWithCta.ctaAction && !showInviteInput && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-primary text-white text-xs font-medium rounded-lg hover:bg-[#25785A] transition-colors disabled:opacity-50"
            >
              {actionLoading ? (
        ...
        {progress.portal_enabled && (
          <a
            ...
            className="flex items-center justify-between px-3 py-2 bg-surface-muted rounded-lg hover:bg-border transition-colors group"
          >
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-green-500" />
              <span className="text-xs font-medium text-text-body">Portal: Enabled</span>
            </div>
            <ExternalLink className="h-3.5 w-3.5 text-text-placeholder group-hover:text-brand-primary transition-colors" />
          </a>
        )}
        ...
        <button
          onClick={loadData}
          className="flex items-center gap-1.5 text-[11px] text-text-placeholder hover:text-text-body transition-colors mx-auto"
        >
        ...
        {history && history.touchpoints.length > 0 && (
          <div>
            <p className="text-[11px] font-semibold text-text-placeholder uppercase tracking-wide mb-2">
              Recent
            </p>
              ...
              {history.touchpoints.slice(0, 3).map((tp) => (
                <div key={tp.id} className="flex items-start gap-2 text-xs">
                  <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${
                    ...
                    tp.status === 'in_progress' ? 'bg-amber-500' : 'bg-border'
                  }`} />
                  <div className="min-w-0">
                    <p className="text-text-body truncate">{tp.title}</p>
        ...
        {history && (history.total_questions_answered > 0 || history.total_documents_received > 0) && (
          <div className="bg-surface-muted rounded-lg p-2.5">
            <p className="text-[11px] font-semibold text-text-placeholder uppercase tracking-wide mb-1.5">
              All Time
            </p>
  ...
  return (
    <button
      ...
      className={`rounded-lg p-2 text-center w-full transition-colors
        ${highlight ? 'bg-amber-50 border border-amber-200' : 'bg-surface-muted'}
        ${onClick ? 'cursor-pointer hover:ring-1 hover:ring-brand-primary/30' : ''}`}
    >
      <div className={`flex items-center justify-center gap-1 ${highlight ? 'text-amber-700' : 'text-text-body'}`}>
        {icon}
        <span className="text-sm font-semibold">{value}</span>
      </div>
      <p className={`text-[10px] ${highlight ? 'text-amber-600' : 'text-text-placeholder'}`}>{label}</p>
    </button>
  )
}
