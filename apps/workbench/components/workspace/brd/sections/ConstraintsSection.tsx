  ...
  return (
    <div className={`bg-white rounded-2xl shadow-md overflow-hidden ${
      ...
      >
        ...
        <span className="text-[14px] font-semibold text-text-body truncate flex-1">{constraint.title}</span>
        {isAiInferred && (
          <span className="inline-flex items-center gap-1 text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-[#E8F5E9] text-brand-primary shrink-0">
          ...
          <div className="px-5 pb-5 pt-1 border-t border-border">
            ...
            {constraint.impact_description && (
              <div className="bg-surface-muted rounded-lg p-3 mb-3">
                <p className="text-[11px] font-medium text-text-placeholder uppercase tracking-wide mb-1">Impact if Ignored</p>
    ...
    <section id="brd-section-constraints">
      <div className="flex items-center justify-between mb-4">
        ...
        {onInferConstraints && (
          <button
            onClick={handleInfer}
            disabled={inferring}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium text-brand-primary bg-[#E8F5E9] rounded-xl hover:bg-[#d4edda] transition-colors disabled:opacity-50"
          >
      ...
      {constraints.length === 0 ? (
        <div className="text-center py-6">
          <Shield className="w-6 h-6 text-text-placeholder mx-auto mb-2" />
          <p className="text-[13px] text-text-placeholder">No constraints identified yet</p>
          {onInferConstraints && (
            <button
              onClick={handleInfer}
              disabled={inferring}
              className="mt-3 inline-flex items-center gap-1.5 px-4 py-2 text-[12px] font-medium text-white bg-brand-primary rounded-xl hover:bg-[#25785A] transition-colors disabled:opacity-50"
            >
            ...
            return (
              <div key={type}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[11px] text-text-placeholder">({items.length})</span>
                </div>
