    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Discovery Prep Review" size="xl" footer={footer}>
      {loading ? (
        ...
        <div className="text-center py-16">
          <HelpCircle className="h-10 w-10 text-ui-supportText/40 mx-auto mb-3" />
          <p className="text-ui-bodyText font-medium mb-1">No prep generated yet</p>
          ...
          <button
            ...
            disabled={generating}
            className="inline-flex items-center gap-2 px-4 py-2 bg-brand-teal text-white text-sm font-medium rounded-lg hover:bg-brand-tealDark disabled:opacity-50 transition-colors"
          >
            {generating ? (
        ...
        <div className="space-y-6 max-h-[60vh] overflow-y-auto">
          ...
          {bundle.agenda_summary && (
            <div className="bg-ui-background rounded-lg p-4">
              <h3 className="text-sm font-semibold text-ui-headingDark mb-2">Agenda Summary</h3>
              <p className="text-sm text-ui-bodyText mb-2">{bundle.agenda_summary}</p>
              {bundle.agenda_bullets.length > 0 && (
                <ul className="list-disc list-inside space-y-1">
                  {bundle.agenda_bullets.map((bullet, i) => (
                    <li key={i} className="text-sm text-ui-bodyText">{bullet}</li>
                  ))}
                </ul>
              )}
            </div>
          ...
          {totalQuestions > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-ui-headingDark mb-3 flex items-center gap-2">
          ...
          {/* Regenerate */}
          {!isSent && (
            <div className="pt-2 border-t border-ui-cardBorder">
              <button
                onClick={handleGenerate}
                disabled={generating}
                className="flex items-center gap-1.5 text-sm text-ui-supportText hover:text-brand-teal transition-colors"
              >
                {generating ? (
...
function QuestionRow({
  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-lg border ${
      question.confirmed ? 'border-green-200 bg-green-50/50' : 'border-ui-cardBorder bg-white'
    }`}>
      <button
        onClick={onToggle}
        disabled={disabled}
      ...
      >
        {toggling ? (
          <Loader2 className="h-5 w-5 text-ui-supportText animate-spin" />
        ) : question.confirmed ? (
          <CheckCircle className="h-5 w-5 text-green-600" />
        ) : (
          <XCircle className="h-5 w-5 text-ui-cardBorder hover:text-red-400 transition-colors" />
        )}
      </button>
      <div className="flex-1 min-w-0">
        <p className="text-sm text-ui-bodyText">{question.question}</p>
        <div className="flex items-center gap-3 mt-1.5">
          <span className="inline-flex items-center gap-1 text-xs text-ui-supportText">
            <User className="h-3 w-3" />
            {question.best_answered_by}
          </span>
        </div>
        {question.why_important && (
          <p className="text-xs text-ui-supportText mt-1">{question.why_important}</p>
        )}
      </div>
    </div>
  )
...
function DocumentRow({
  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-lg border ${
      document.confirmed ? 'border-green-200 bg-green-50/50' : 'border-ui-cardBorder bg-white'
    }`}>
      <button
        onClick={onToggle}
        disabled={disabled}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-sm font-medium text-ui-bodyText">{document.document_name}</p>
        ...
        {document.why_important && (
          <p className="text-xs text-ui-supportText mt-1">{document.why_important}</p>
        )}
      </div>
    </div>
  )
