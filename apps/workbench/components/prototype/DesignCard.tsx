  ...
  return (
    <button
      ...
      className={`
        ...
        ${
          isSelected
            ? 'border-brand-primary bg-brand-primary-light ring-1 ring-brand-primary/20'
    ...
    >
      ...
      {/* Label */}
      <div className={`font-medium text-text-body ${isSmall ? 'text-sm' : 'text-base'}`}>
        {label}
      </div>

      {/* Description */}
      <p className={`text-text-placeholder mt-0.5 ${isSmall ? 'text-xs' : 'text-sm'}`}>
        {description}
      </p>

      {/* Source attribution */}
      {source && (
        <p className="text-xs text-text-placeholder mt-1.5 italic">
          Source: {source}
        </p>
      )}
    </button>
