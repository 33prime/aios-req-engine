          ...
          {chat.isComplete ? (
            <div className="flex items-center justify-end gap-2.5">
              <button
                ...
                className="px-4 py-2.5 text-[13px] font-medium text-[#666666] bg-[#F0F0F0] rounded-2xl hover:bg-border transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isGenerating}
                className="px-4 py-2.5 text-[13px] font-medium text-white bg-brand-primary rounded-2xl hover:bg-[#25785A] transition-colors disabled:opacity-50 flex items-center gap-1.5"
              >
                {isGenerating ? (
            ...
            <p className="text-center text-[12px] text-text-placeholder">
...
function MessageBubble({
  ...
  return (
    <div className="flex items-start">
      <div className="bg-white border border-border rounded-2xl rounded-bl-md px-4 py-3 shadow-sm max-w-[90%]">
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 px-3 py-2 text-[12px] font-medium text-white bg-brand-primary rounded-lg hover:bg-[#25785A] transition-colors"
        >
          Use my brand
        </button>
        <button
          onClick={() => onSelect('generic')}
          className="flex-1 px-3 py-2 text-[12px] font-medium text-[#666666] bg-white border border-border rounded-lg hover:bg-[#F4F4F4] transition-colors"
        >
          Choose a style
        </button>
      </div>
  ...
  return (
    <div className="space-y-2">
        ...
        <button
          ...
          className={`w-full text-left p-2.5 rounded-lg border transition-all flex items-center gap-2 ${
            selected.has(item.id)
              ? 'border-brand-primary bg-[#E8F5E9]/50'
              : 'border-border bg-white hover:border-brand-primary/30'
          }`}
        >
          <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${
            selected.has(item.id) ? 'border-brand-primary bg-brand-primary' : 'border-border'
          }`}>
            {selected.has(item.id) && <Check className="w-3 h-3 text-white" />}
          </div>
          <div>
            <p className="text-[12px] font-medium text-text-body">{item.name}</p>
            <p className="text-[10px] text-text-placeholder">{item.subtitle}</p>
          </div>
        </button>
      ))}
      <div className="flex gap-2 pt-1">
        ...
        <button
          onClick={() => onConfirm([])}
          className="px-3 py-2 text-[12px] font-medium text-text-placeholder hover:text-[#666666] transition-colors"
        >
          Skip
        </button>
      </div>
...
function TokenEditorInline({
  ...
  return (
    <div className="space-y-3 bg-white rounded-lg border border-border p-3">
      {/* Colors */}
      <div>
        <p className="text-[11px] font-medium text-text-placeholder uppercase tracking-wider mb-1.5">Colors</p>
        <div className="flex gap-3">
          {(['primary_color', 'secondary_color', 'accent_color'] as const).map((key) => (
            <div key={key} className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-text-placeholder">{key.replace('_color', '').replace('_', ' ')}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <p className="text-[10px] font-medium text-text-placeholder uppercase tracking-wider mb-1">Heading Font</p>
          <select
            value={localTokens.font_heading}
            onChange={(e) => handleChange('font_heading', e.target.value)}
            className="w-full text-[12px] px-2 py-1.5 border border-border rounded-lg text-text-body focus:outline-none focus:ring-1 focus:ring-brand-primary"
          >
            {FONT_OPTIONS.map((f) => <option key={f} value={f}>{f}</option>)}
          </select>
        </div>
        <div>
          <p className="text-[10px] font-medium text-text-placeholder uppercase tracking-wider mb-1">Body Font</p>
          <select
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          <p className="text-[10px] font-medium text-text-placeholder uppercase tracking-wider mb-1">Spacing</p>
          <div className="flex gap-1">
              ...
              <button
                ...
                className={`flex-1 px-2 py-1 text-[10px] font-medium rounded-md transition-colors ${
                  localTokens.spacing === opt
                    ? 'bg-[#E8F5E9] text-[#25785A] border border-brand-primary/20'
                    : 'bg-[#F0F0F0] text-text-placeholder border border-transparent hover:bg-border'
                }`}
              >
                {opt}
              </button>
        ...
        <div>
          <p className="text-[10px] font-medium text-text-placeholder uppercase tracking-wider mb-1">Corners</p>
          <div className="flex gap-1">
      ...
      <button
        onClick={onConfirm}
        className="w-full px-3 py-2 text-[12px] font-medium text-white bg-brand-primary rounded-lg hover:bg-[#25785A] transition-colors flex items-center justify-center gap-1.5"
      >
  ...
  return (
    <div className="bg-white rounded-lg border border-border p-3 space-y-2">
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[11px]">
        <div><span className="text-text-placeholder">Heading:</span> <span className="text-text-body">{t.font_heading}</span></div>
        <div><span className="text-text-placeholder">Body:</span> <span className="text-text-body">{t.font_body}</span></div>
        <div><span className="text-text-placeholder">Spacing:</span> <span className="text-text-body">{t.spacing}</span></div>
        <div><span className="text-text-placeholder">Corners:</span> <span className="text-text-body">{t.corners}</span></div>
      </div>
      {t.logo_url && (
        <div className="pt-1">
          {/* eslint-disable-next-line @next/next/no-img-element */}
