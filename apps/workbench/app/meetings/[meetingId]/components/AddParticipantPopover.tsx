  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button className="w-7 h-7 rounded-md border border-dashed border-[#D0D0D0] flex items-center justify-center text-[#7B7B7B] hover:border-[#044159] hover:text-[#044159] hover:bg-[#f0f7fa] transition-colors">
        ...
        {!showCreate ? (
          <div>
            <div className="p-3 border-b border-[#F0F0F0]">
              <div className="relative">
                ...
                <input
                  ...
                  autoFocus
                  className="w-full pl-8 pr-3 py-1.5 text-[13px] border border-[#E5E5E5] rounded-md bg-[#FAFAFA] outline-none focus:border-[#88BABF] focus:bg-white"
                />
              </div>
            </div>
            ...
            <div className="max-h-[240px] overflow-y-auto">
              {loading ? (
                <div className="flex items-center justify-center py-8">
                  <Loader2 className="w-4 h-4 animate-spin text-[#7B7B7B]" />
                </div>
              ) : available.length > 0 ? (
                available.map((s) => (
                  <button
                    ...
                    disabled={adding}
                    className="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-[#FAFAFA] transition-colors disabled:opacity-50"
                  >
                    <div className="w-6 h-6 rounded-full bg-[#044159] flex items-center justify-center text-[9px] font-bold text-white flex-shrink-0">
                      {s.name.split(' ').map((w) => w[0]).join('').toUpperCase().slice(0, 2)}
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="text-[13px] font-medium text-[#1D1D1F] truncate">{s.name}</div>
                      <div className="text-[11px] text-[#7B7B7B] truncate">
                ...
                <div className="py-6 text-center text-[13px] text-[#7B7B7B]">
            ...
            <div className="p-2 border-t border-[#F0F0F0]">
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-[12px] font-medium text-[#044159] rounded-md hover:bg-[#f0f7fa] transition-colors"
              >
                <Plus className="w-3.5 h-3.5" />
          ...
          <div className="p-3">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[13px] font-semibold text-[#1D1D1F]">New Stakeholder</span>
              <button onClick={() => setShowCreate(false)} className="p-1 rounded hover:bg-[#F0F0F0]">
                <X className="w-3.5 h-3.5 text-[#7B7B7B]" />
              </button>
            </div>
            ...
            <div className="space-y-2.5">
              <input
                ...
                placeholder="Name *"
                autoFocus
                className="w-full px-2.5 py-1.5 text-[13px] border border-[#E5E5E5] rounded-md outline-none focus:border-[#88BABF]"
              />
              <input
                type="email"
                value={newEmail}
              ...
              <select
                ...
                className="w-full px-2.5 py-1.5 text-[13px] border border-[#E5E5E5] rounded-md outline-none focus:border-[#88BABF] bg-white"
              >
              ...
              <button
                ...
                className="w-full py-1.5 text-[12px] font-medium text-white bg-[#044159] rounded-md hover:bg-[#033344] transition-colors disabled:opacity-50"
              >
