 ...
 *
 * Tab 4: Actions (Client Confirmations & Meeting Agendas)
 *
 ...
 * - Generate meeting agendas from selected confirmations
 */
...
import { TwoColumnLayout, Button } from '@/components/ui'
...
import { MeetingAgendaDisplay } from '@/components/meeting/MeetingAgendaDisplay'
import {
  listConfirmations,
...
import type { MeetingAgenda } from '@/lib/export-utils'

interface ActionsTabProps {
  projectId: string
}

export function ActionsTab({ projectId }: ActionsTabProps) {
  ...
  const handleGenerateMeetingAgenda = async (confirmationIds: string[]) => {
    try {
      ...
      alert('Failed to generate meeting agenda. Please try again.')
    }
  }

  // Close meeting agenda
  const handleCloseMeetingAgenda = () => {
  ...
  // Count open/queued confirmations
  const availableForMeeting = confirmations.filter(
  ...
  return (
    <>
      <div className="p-6">
        {/* Header with Meeting Generation Button */}
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h2 className="heading-2">Client Actions</h2>
            <p className="text-support text-ui-supportText mt-1">
              Manage confirmations and generate meeting agendas
            </p>
          </div>
          <Button
            onClick={() => setShowAgendaBuilder(true)}
            disabled={availableForMeeting === 0}
            title={
              availableForMeeting === 0
                ? 'No confirmations available for meeting'
                : 'Generate meeting agenda from confirmations'
            }
          >
      ...
      {meetingAgenda && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-lg max-w-4xl w-full my-8 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <MeetingAgendaDisplay agenda={meetingAgenda} onClose={handleCloseMeetingAgenda} />
            </div>
          </div>
        </div>
      )}
