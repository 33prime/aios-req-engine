      ...
      >
        {loading ? (
          <div className="p-4 text-center">
            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-[#009b87] mx-auto mb-2" />
            <p className="text-xs text-ui-supportText">Loading criteria...</p>
          </div>
        ) : error && !status ? (
          <div className="p-4 text-center">
          ...
          <div>
            ...
            <div className="px-4 pt-3 pb-2 border-b border-ui-cardBorder">
              <div className="flex items-center gap-1.5 text-sm font-medium text-ui-headingDark">
                <span>{STAGE_LABELS[status.current_stage] || status.current_stage}</span>
                <svg className="w-3.5 h-3.5 text-ui-supportText" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {status.transition_description && (
                <p className="text-xs text-ui-supportText mt-0.5">{status.transition_description}</p>
              )}
            </div>
            ...
            {status.criteria_total > 0 && (
              <div className="px-4 pt-2">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-medium text-ui-headingDark">
            ...
            {/* Actions */}
            <div className="px-4 py-3 border-t border-ui-cardBorder space-y-2">
              {status.can_advance && status.next_stage && (
                <button
                  onClick={() => handleAdvance(false)}
                  disabled={advancing}
                  className="w-full px-3 py-1.5 text-xs font-medium text-white bg-[#009b87] rounded-md hover:bg-[#008574] disabled:opacity-50 transition-colors"
                >
                ...
                <>
                  {!showForce ? (
                    <button
                      ...
                      className="w-full text-xs text-ui-supportText hover:text-ui-headingDark transition-colors"
                    >
                      Override...
                    </button>
                  ) : (
                    <div className="space-y-1.5">
                      <input
                        ...
                        className="w-full px-2 py-1 text-xs border border-ui-cardBorder rounded-md focus:outline-none focus:ring-1 focus:ring-[#009b87]"
                      />
                      <button
  ...
  return (
    <div>
      ...
      >
        ...
        <span className={`text-xs ${criterion.satisfied ? 'text-ui-headingDark' : 'text-ui-supportText'}`}>
          {criterion.gate_label}
        </span>
        {hasDetails && (
          <svg
            className={`w-3 h-3 text-ui-supportText ml-auto mt-0.5 flex-shrink-0 transition-transform ${expanded ? 'rotate-90' : ''}`}
      ...
      {expanded && hasDetails && (
        <div className="ml-6 mt-1 space-y-1">
          {criterion.missing.length > 0 && (
            <div>
              <p className="text-[10px] font-medium text-ui-supportText uppercase tracking-wide">Missing</p>
              <ul className="text-[11px] text-ui-bodyText space-y-0.5">
          ...
          {criterion.how_to_acquire.length > 0 && (
            <div>
              <p className="text-[10px] font-medium text-ui-supportText uppercase tracking-wide">How to acquire</p>
              <ul className="text-[11px] text-ui-bodyText space-y-0.5">
                {criterion.how_to_acquire.map((h, i) => (
                  <li key={i} className="flex items-start gap-1">
                    <span className="text-[#009b87] mt-px">+</span>
                    <span>{h}</span>
                  </li>
                ))}
              </ul>
