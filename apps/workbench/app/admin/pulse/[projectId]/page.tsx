...
const DIRECTIVE_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  stable: { bg: 'bg-brand-primary/15', text: 'text-[#25785A]', label: 'Stable' },
...
export default function AdminPulseDetailPage() {
  ...
  if (!latest) {
    return (
      <div className="space-y-4">
        <Link href="/admin/pulse" className="flex items-center gap-1.5 text-[13px] text-brand-primary hover:text-[#25785A]">
          <ArrowLeft className="w-3.5 h-3.5" /> Back to Pulse
        </Link>
        <p className="text-[13px] text-text-placeholder">No pulse data for this project yet.</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <Link href="/admin/pulse" className="flex items-center gap-1.5 text-[13px] text-brand-primary hover:text-[#25785A] mb-3">
          <ArrowLeft className="w-3.5 h-3.5" /> Back to Pulse
        </Link>
        <div className="flex items-center gap-3">
          <h1 className="text-[22px] font-bold text-text-body">{projectName || projectId.slice(0, 8)}</h1>
          <span className={`inline-block px-2.5 py-0.5 rounded-full text-[11px] font-medium ${STAGE_COLORS[latest.stage] || 'bg-border text-[#666666]'}`}>
            {latest.stage}
          </span>
        </div>
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          <span className="text-[12px] text-text-placeholder font-mono">
            ...
            {latest.gates.map((gate, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-2 text-[12px]">
                  ...
                  <span className={met ? 'text-[#666666]' : 'text-text-body font-medium'}>
                    {gate.replace(/^\[.\]\s*/, '')}
                  </span>
                </div>
              )
      ...
      {/* Entity health cards */}
      <div>
        <h2 className="text-[15px] font-semibold text-text-body mb-3">Entity Health</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {healthEntries.map(([entityType, health]) => {
            ...
            return (
              <div key={entityType} className="bg-white rounded-xl border border-border p-4">
                ...
                <div className="flex items-baseline gap-1 mb-2">
                  <span className="text-[20px] font-bold text-text-body">{health.count}</span>
                  <span className="text-[12px] text-text-placeholder">/ {health.target}</span>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-border p-4">
          <h3 className="text-[13px] font-semibold text-text-body mb-3 flex items-center gap-1.5">
            <Zap className="w-3.5 h-3.5 text-brand-primary" /> Actions
          </h3>
          <div className="space-y-2.5">
            {(latest.actions || []).map((action, i) => (
              <div key={i} className="flex items-start gap-2">
                <div className="flex-1">
                  <p className="text-[12px] text-text-body">{action.sentence}</p>
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[10px] text-text-placeholder font-mono">{action.impact_score.toFixed(0)}</span>
            ...
            {(!latest.actions || latest.actions.length === 0) && (
              <p className="text-[12px] text-text-placeholder">No actions needed</p>
            )}
          </div>
        </div>
          ...
          {latest.risks && (
            <div className="space-y-3">
              <div className="flex items-baseline gap-2">
                <span className="text-[24px] font-bold text-text-body">
                  {latest.risks.risk_score.toFixed(0)}
                </span>
                <span className="text-[12px] text-text-placeholder">/ 100</span>
              </div>
              <div className="space-y-1.5 text-[12px]">
                <div className="flex justify-between">
                  <span className="text-[#666666]">Stale clusters</span>
                  <span className="font-medium text-text-body">{latest.risks.stale_clusters}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#666666]">Critical questions</span>
                  <span className="font-medium text-text-body">{latest.risks.critical_questions}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#666666]">Single-source types</span>
                  <span className="font-medium text-text-body">{latest.risks.single_source_types}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#666666]">Contradictions</span>
                  <span className="font-medium text-text-body">{latest.risks.contradiction_count}</span>
                </div>
              </div>
            </div>
          )}
        ...
        {/* Forecast */}
        <div className="bg-white rounded-xl border border-border p-4">
          <h3 className="text-[13px] font-semibold text-text-body mb-3">Forecast</h3>
          {latest.forecast && (
            <div className="space-y-3">
              {[
                ...
                <div key={metric.label}>
                  <div className="flex justify-between text-[12px] mb-1">
                    ...
                    <span className="font-medium text-text-body">{(metric.value * 100).toFixed(0)}%</span>
                  </div>
      ...
      <div className="bg-white rounded-xl border border-border p-4">
        <h3 className="text-[13px] font-semibold text-text-body mb-3 flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5 text-text-placeholder" /> Snapshot Timeline
        </h3>

        {history.length === 0 ? (
          <p className="text-[12px] text-text-placeholder">No snapshots recorded yet.</p>
        ) : (
              ...
              return (
                <div key={snap.id || i} className="flex items-start gap-3 py-2 border-b border-[#F4F4F4] last:border-0">
                  ...
                  <div className="flex flex-col items-center pt-1">
                    <div className={`w-2.5 h-2.5 rounded-full ${stageChanged ? 'bg-brand-primary' : 'bg-border'}`} />
                  ...
                  <div className="flex-1 flex items-center gap-3 text-[12px]">
                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${STAGE_COLORS[snap.stage] || 'bg-border text-[#666666]'}`}>
                      {snap.stage}
                    </span>
                    ...
                    {stageChanged && (
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-brand-primary/15 text-[#25785A]">
                        stage change
                      </span>
                    )}
                    <span className="text-text-placeholder ml-auto">
