  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-[22px] font-bold text-text-body">Projects</h1>
      ...
      <div className="bg-white rounded-2xl shadow-md border border-border overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-[#F8F9FB]">
                <th className="text-left text-[11px] text-text-placeholder uppercase tracking-wide px-4 py-3">Name</th>
                <th className="text-left text-[11px] text-text-placeholder uppercase tracking-wide px-4 py-3">Client</th>
                <th className="text-left text-[11px] text-text-placeholder uppercase tracking-wide px-4 py-3">Owner</th>
                <th className="text-left text-[11px] text-text-placeholder uppercase tracking-wide px-4 py-3">Stage</th>
                <th className="text-right text-[11px] text-text-placeholder uppercase tracking-wide px-4 py-3">Signals</th>
                <th className="text-right text-[11px] text-text-placeholder uppercase tracking-wide px-4 py-3">Features</th>
                <th className="text-left text-[11px] text-text-placeholder uppercase tracking-wide px-4 py-3">Created</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((project) => (
                <tr key={project.id} className="border-b border-border hover:bg-[#F4F4F4] transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/projects/${project.id}`} className="text-[13px] text-brand-primary hover:underline font-medium">
                      {project.name}
                    </Link>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-brand-primary to-[#25785A] flex items-center justify-center overflow-hidden flex-shrink-0">
                  ...
                  <td className="px-4 py-3 text-[13px] text-text-body text-right">{project.signal_count}</td>
                  <td className="px-4 py-3 text-[13px] text-text-body text-right">{project.feature_count}</td>
                  <td className="px-4 py-3 text-[12px] text-text-placeholder">{new Date(project.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12 text-text-placeholder text-sm">No projects found</div>
        )}
      </div>
    </div>
  )
