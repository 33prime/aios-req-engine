...
def build_facts_prompt(
    ...
    if project_context and project_context.get("state_snapshot"):
        ...
        lines.append("8. EVERY named process in the signal MUST become a workflow with 3-8+ steps")
        lines.append("9. Business requirements (BRs) are BOTH features AND workflow steps — extract as BOTH")
        lines.append("10. INFER current-state steps for future workflows (mark as confidence: low)")
        lines.append("")
    ...
    # Fallback project context if no state snapshot
        ...
        lines.append("5. Extract ALL workflow steps. Group by workflow_name. Use CURRENT_PROCESS/FUTURE_PROCESS fact types.")
        lines.append("6. Extract ALL business drivers (pains, goals, KPIs)")
        lines.append("7. Extract ALL data entities (domain objects: records, documents, forms)")
        lines.append("8. EVERY named process MUST become a workflow with 3-8+ steps")
        lines.append("9. Business requirements (BRs) are BOTH features AND workflow steps — extract as BOTH")
        lines.append("")
        lines.append("DO NOT skip extraction just because something similar exists.")
