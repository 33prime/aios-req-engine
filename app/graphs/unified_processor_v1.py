"""
DEPRECATED: V1 Unified Signal Processor.

Uses SimilarityMatcher-based claim extraction. Superseded by V2 (EntityPatch pipeline)
in unified_processor.py. No production callers — kept for reference only.

Original entry point: process_signal(signal_id, project_id, run_id)
"""
    ...
    try:
        if entity_type == "feature":
            ...
            if claim.field_hints:
                for field_name, value in claim.field_hints.items():
                    if field_name in details:
                        if isinstance(details[field_name], list):
                            details[field_name].append(value)
                        else:
...
def process_signal(
    ...
    """DEPRECATED: Use process_signal_v2 from unified_processor.py instead."""
    logger.warning("V1 process_signal called — this is deprecated, use process_signal_v2")
    ...
    try:
        ...
        logger.exception(f"V1 processing failed: {e}")
        return ProcessingResult(
            signal_id=str(signal_id),
            project_id=str(project_id),
            success=False,
...
async def process_signal_async(
    ...
    """DEPRECATED: Async wrapper for V1 process_signal."""
    import asyncio
