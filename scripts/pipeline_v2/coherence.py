...
def _format_context(
    ...
    # Features
    h1 = [f for f in payload.features if f.horizon == "H1"]
    h2 = [f for f in payload.features if f.horizon == "H2"]
    h3 = [f for f in payload.features if f.horizon == "H3"]
    lines.append(f"## Features ({len(payload.features)}: {len(h1)} H1, {len(h2)} H2, {len(h3)} H3)")
